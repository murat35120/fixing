...
trb=[2,tp,3];//внешний, внутренний высота
knt=[20,11, 7.5, 7.8];//высота до центра, толщина, высота стопора

dkn=2.5;//длина контактной площадки
shp=2.0;//высота шипа
do=3.8;
step=3.5;//шаг контактов
lin=[step-1.2,knt.x+trb.x,2];//ширина, длина, толщина
kk=2;//количество контактов
hhr=5+0.5;//высота верхней части раъема 0.5 это тлщина провода
//прищепка
...
module alll(){
    difference(){
        ...
        translate([-prk[3]*2+prk[3]-ts/2,0,0]){
            translate([0,prk.y/2,tpr/2+prk[4]/3])cube([prk[3]*4,tpr,tpr],true);
            translate([0,prk.y/2,tpr/2+2*prk[4]/3])cube([prk[3]*4,tpr,tpr],true);
            translate([0,-prk.y/2,tpr/2+prk[4]/3])cube([prk[3]*4,tpr,tpr],true);
            translate([0,-prk.y/2,tpr/2+2*prk[4]/3])cube([prk[3]*4,tpr,tpr],true);
        }
    ...
    difference(){
        union(){
            ...
            translate([0, -step*(kk-1)/2, 0]) {
                for(a=[0:1:kk-1]){
                    translate([0,a*step,0]){
                        translate([lin.y+prk[3],0,0])rotate([0,0,-90])pin_lin_0();
                        translate([prk[3],0,knt[3]/2])cube([ts,step+1,knt[3]],true);
                        translate([prk[3]-knt[3]/2,0,ts/2])cube([knt[3],step+1,ts],true);
                    }
                }
...
module pin_lin(){
    difference(){
        union(){
            ...
            translate([0, -trb.z/2+trb.y-dkn/2-ts/2-hhr,(trb.z+lin.z+ts)/2]) cube([lin.x,ts,trb.z+lin.z+ts],true);
...
module pin_lin_0(){
    difference(){
        union(){
            ...
            //translate([0, -trb.z/2+trb.y,(trb.z+lin.z)/2]) cube([lin.x,dkn,trb.z+lin.z],true);
           // translate([0, -trb.z/2+trb.y-dkn/2-ts/2-hhr,(trb.z+lin.z+ts)/2]) cube([lin.x,ts,trb.z+lin.z+ts],true);
            translate([0, trb.x/2-lin.y/2-0.5,lin.z]) cube([lin.x, lin.y, lin.z*2],true);
        }
        //translate([0,-0.25,-0.1])cylinder(trb.z*2,trb.y/2,trb.y/2);
        //translate([0,-0.25,trb.z-0.1])cube([trb.y,trb.y,trb.z*2],true);
        //translate([0,-trb.z+0.25,trb.z-0.1])cube([trb.y,trb.y,trb.z*2],true);

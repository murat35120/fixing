...
do=3.8;
guard=[15,8.5,11];
step=guard.x;//шаг контактов
lin=[step,knt.x+trb.x,4];//ширина, длина, толщина
kk=1;//количество контактов
hhr=4.5+0.5;//высота верхней части раъема
//прищепка
...
module pin_lin(){
    difference(){
      ...
      //  translate([0, 0,trb.z+lin.z+trb.x/4]) rotate([-45,0,0])cube([trb.x*2,trb.x*3,trb.x/2],true);
        translate([0, -trb.z/2+trb.y-dkn/2-hhr-ts+guard.z,ts]) cube([guard.y,guard.z*2,ts*5],true);
    }
}
module pin_lin_0(){
    difference(){

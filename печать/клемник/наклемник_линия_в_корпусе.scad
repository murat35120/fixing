...
knt=[12,11, 7.5, 7.8];//высота до центра, толщина, высота стопора
...
module pin_lin(){
    difference(){
        union(){
            //cylinder(trb.z+lin.z,trb.x/2,trb.x/2);
            translate([0, -trb.z/2,(trb.z+lin.z)/2]) cube([lin.x,trb.z*2,trb.z+lin.z],true);
            translate([0, trb.x/2-lin.y/2,lin.z/2]) cube(lin,true);
        }
        translate([0,0,-0.1])cylinder(trb.z*2,trb.y/2,trb.y/2);
        translate([0,-trb.z,-0.1])cylinder(trb.z*2,trb.y/2,trb.y/2);
        translate([0, 0,trb.z+lin.z+trb.x/4]) rotate([-45,0,0])cube([trb.x*2,trb.x*3,trb.x/2],true);
    }
}
module pin_s(){
